...
-(void)setRGBAToBitmapData:(BitmapPixel)pixel atPointX:(long)pointX atPointY:(long)pointY
{
    ...
    unsigned char *argbData = &bitmapData[((pointY * width) + pointX) * 4];
 ...
 *
 */
-(void)genBitmapDataToColorWheel:(CGRect)imageFrame
{
    CGFloat imageWidth = imageFrame.size.width;
    CGFloat imageHeight = imageFrame.size.height;
    CGFloat radius = imageFrame.size.width / 2;
...
-(UIImage *)createColorWheelImageWithFrame:(CGRect)imageFrame
{
    ...
    bitmapContext = [self newARGBBitmapContextWithSize:CGSizeMake(imageWidth, imageHeight)];
    bitmapData = CGBitmapContextGetData(bitmapContext);
    [self genBitmapDataToColorWheel:imageFrame];
...
-(UIImage *)createColorTempImageWithFrame:(CGRect)imageFrame
{
    ...
    CGGradientRef gradient =
            CGGradientCreateWithColorComponents(colorTemp,
                                                components,
                                                locations,
                                                colorCount);
...
-(UIImage *)createColorBrightnessImageWithFrame:(CGRect)imageFrame
{
    ...
    CGGradientRef gradient =
            CGGradientCreateWithColorComponents(brigtnessColor,
                                                components,
                                                locations,
                                                colorCount);
...
{
    CGPoint centerPoint = CGPointMake((imageFrame.size.width) / 2, (imageFrame.size.height / 2));
